# NakPom — Database Documentation (Sprint 1 & Sprint 2)
...
Stores time-limited tokens for password reset flows. The complete backend flow is now implemented in Sprint 2:
- `POST /api/v1/auth/forgot-password` → generates token, saves here, sends email
- `POST /api/v1/auth/reset-password` → validates token + expiry, updates `users.password_hash`, deletes row
...
| **Data Isolation** | Family feeds are gated by `family_memberships` — a user without a membership row for a given `family_id` cannot access that family's data. This is enforced at the application layer by `FamilyMembershipInterceptor` before any controller logic runs. |
...
| **Reset Token Lifetime** | `expires_at` column enforces time-limited tokens. Application logic rejects expired tokens and deletes them on use. Token is also overwritten (old token deleted) on each new reset request to prevent accumulation. |
...
## 8. Sprint 2 — Schema Changes

**No new Flyway migrations required for Sprint 2.**

All tables needed for Sprint 2 features already exist from Sprint 1:

| Sprint 2 Feature | Tables Used | Notes |
|---|---|---|
| Password Reset (`forgot-password`, `reset-password`) | `users`, `password_resets` | Token is written on request, deleted on use or overwrite |
| Family Invite Engine (`joinFamilyByCode`) | `families`, `family_memberships` | New `role="member"` row inserted on successful join |
| Membership Security Filter (`FamilyMembershipInterceptor`) | `family_memberships` | Uses `existsByUserIdAndFamilyId()` — a single indexed COUNT query |

### Security-Critical Query: `existsByUserIdAndFamilyId()`

```sql
-- Generated by Spring Data JPA from the method name:
SELECT COUNT(*) > 0
FROM family_memberships
WHERE user_id = ? AND family_id = ?
```

This query is called on **every request** to `/api/v1/family/{familyId}/**` by the `FamilyMembershipInterceptor`. It is the enforcement point for the entire data isolation model. Performance characteristics:

| Factor | Detail |
|---|---|
| **Index used** | Composite unique index on `(user_id, family_id)` — O(log n) lookup |
| **Result** | Boolean — Spring Data returns `true`/`false` directly |
| **Frequency** | Once per protected request, before any data is fetched |
| **Failure mode** | Returns `false` → 403 Forbidden, controller never executes |
